...
stop_words = ["0o", "0s", "3a", "3b", "3d", "6b", "6o", "a", "a1", "a2", "a3", "a4", "ab", "able", "about", "above",
              ...
              "bp", "br", "brief", "briefly", "bs", "bt", "bu", "but", "bx", "by", "c1", "c2", "c3", "ca", "call",
              ...
              "cs", "c's", "ct", "cu", "currently", "cv", "cx", "cy", "cz", "d2", "da", "date", "dc", "dd", "de",
              ...
              "oq", "or", "ord", "os", "ot", "other", "others", "otherwise", "ou", "ought", "our", "ours", "ourselves",
              ...
              "yours", "yourself", "yourselves", "you've", "yr", "ys", "yt", "z", "zero", "zi", "zz", 'don’t']
...
def tf_idf(corpus_idf, job_tf):

    job_tf_idf = {}

    for word in job_tf:
        if word in corpus_idf:
            word_tf_idf = job_tf[word] * corpus_idf[word]
            job_tf_idf[word] = word_tf_idf

    sorted_tfidf = sorted(job_tf_idf.items(), key=lambda x: x[1], reverse=False)


    # either run idf locally or get it from database
    # (i'm thinking of storing the most recent idf calculation on the database,
    # using it to determine tf-idf then recalculating idf based on new document and replacing current idf with new.)
    # calculate tf of document, determine tf-idf of every term in document. return list with tf-idf
    # after this method returns that list we will display the top results of the list (or bottom depending)
    return sorted_tfidf
...
def data_cleaner(data):
    ...
    Jobs_Test_Clean = str(data['Description'])

    Jobs_Test_Clean = re.sub("\<.*?\>", "  ", Jobs_Test_Clean)

    Jobs_Test_Clean = Jobs_Test_Clean.replace("\n", " ")

    punctuation = '!"$%&\'()*,-./:;<=>?@[\]^_`{|}~'
    translator = str.maketrans(punctuation, ' ' * len(punctuation))  # map punctuation to space
    Jobs_Test_Clean = Jobs_Test_Clean.translate(translator)

    translator = str.maketrans('', '', digits)
    Jobs_Test_Clean = Jobs_Test_Clean.translate(translator)

    Jobs_Test_Clean = " ".join(word for word in Jobs_Test_Clean.split() if word not in stop_words)

    # After consideration i decided that removing these
    # characters wasn't a good idea since things like C, C++, and C# all
    # exsist and are all very different from eachother.

    # Jobs_Test_Clean = re.sub(r" [^a-zA-Z0-9#+ ]", "", Jobs_Test_Clean) # Removes special characters

    Jobs_Test_Clean = re.sub(r'http\S+', '', Jobs_Test_Clean)  # remove any links that might be in the description

    Jobs_Test_Clean = Jobs_Test_Clean.split()

    final_jobs = []

    for word in Jobs_Test_Clean:
        ...
        final_jobs.append(word)

    Jobs_Test_Clean = {'Company': data['Company'], 'Description': final_jobs}
    print(Jobs_Test_Clean)


    # clean_jobs_data = json.dumps(Jobs_Test_Clean, indent=2)

    return Jobs_Test_Clean
